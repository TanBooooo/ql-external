...
def bark(title: str, content: str) -> None:
    ...
    if not push_config.get("BARK_PUSH"):
        log.info("bark 服务的 BARK_PUSH 未设置!!\n取消推送")
        return
    log.info("bark 服务启动")
...
def dingding_bot(title: str, content: str) -> None:
    ...
    if not push_config.get("DD_BOT_SECRET") or not push_config.get("DD_BOT_TOKEN"):
        log.info("钉钉机器人 服务的 DD_BOT_SECRET 或者 DD_BOT_TOKEN 未设置!!\n取消推送")
        return
    log.info("钉钉机器人 服务启动")
...
def go_cqhttp(title: str, content: str) -> None:
    ...
    if not push_config.get("GOBOT_URL") or not push_config.get("GOBOT_QQ"):
        log.info("go-cqhttp 服务的 GOBOT_URL 或 GOBOT_QQ 未设置!!\n取消推送")
        return
...
def gotify(title: str, content: str) -> None:
    ...
    if not push_config.get("GOTIFY_URL") or not push_config.get("GOTIFY_TOKEN"):
        log.info("gotify 服务的 GOTIFY_URL 或 GOTIFY_TOKEN 未设置!!\n取消推送")
        return
...
def iGot(title: str, content: str) -> None:
    ...
    if not push_config.get("IGOT_PUSH_KEY"):
        log.info("iGot 服务的 IGOT_PUSH_KEY 未设置!!\n取消推送")
        return
    log.info("iGot 服务启动")
    ...
    if response["ret"] == 0:
        ...
        log.info(f'iGot 推送失败！{response["errMsg"]}')
...
def serverJ(title: str, content: str) -> None:
    ...
    if not push_config.get("PUSH_KEY"):
        log.info("serverJ 服务的 PUSH_KEY 未设置!!\n取消推送")
        return
    ...
    if response.get("errno") == 0 or response.get("code") == 0:
        ...
        log.info(f'serverJ 推送失败！错误码：{response["message"]}')
...
def pushdeer(title: str, content: str) -> None:
    ...
    if not push_config.get("DEER_KEY"):
        log.info("PushDeer 服务的 DEER_KEY 未设置!!\n取消推送")
        return
    ...
    if len(response.get("content").get("result")) > 0:
        ...
        log.info("PushDeer 推送失败！错误信息：", response)
...
def chat(title: str, content: str) -> None:
    ...
    if not push_config.get("CHAT_URL") or not push_config.get("CHAT_TOKEN"):
        log.info("chat 服务的 CHAT_URL或CHAT_TOKEN 未设置!!\n取消推送")
        return
    log.info("chat 服务启动")
    ...
    if response.status_code == 200:
        ...
        log.info("Chat 推送失败！错误信息：", response)
...
def qmsg_bot(title: str, content: str) -> None:
    ...
    if not push_config.get("QMSG_KEY") or not push_config.get("QMSG_TYPE"):
        log.info("qmsg 的 QMSG_KEY 或者 QMSG_TYPE 未设置!!\n取消推送")
        return
    log.info("qmsg 服务启动")
    ...
    if response["code"] == 0:
        ...
        log.info(f'qmsg 推送失败！{response["reason"]}')
...
def wecom_bot(title: str, content: str) -> None:
    ...
    if not push_config.get("QYWX_KEY"):
        log.info("企业微信机器人 服务的 QYWX_KEY 未设置!!\n取消推送")
        return
...
def telegram_bot(title: str, content: str) -> None:
    ...
    if not push_config.get("TG_BOT_TOKEN") or not push_config.get("TG_USER_ID"):
        log.info("tg 服务的 bot_token 或者 user_id 未设置!!\n取消推送")
        return
    log.info("tg 服务启动")
...
def aibotk(title: str, content: str) -> None:
    ...
    if not push_config.get("AIBOTK_KEY") or not push_config.get("AIBOTK_TYPE") or not push_config.get("AIBOTK_NAME"):
        log.info("智能微秘书 的 AIBOTK_KEY 或者 AIBOTK_TYPE 或者 AIBOTK_NAME 未设置!!\n取消推送")
        return
    log.info("智能微秘书 服务启动")
    ...
    if response["code"] == 0:
        ...
        log.info(f'智能微秘书 推送失败！{response["error"]}')
...
def smtp(title: str, content: str) -> None:
    ...
    使用 SMTP 邮件 推送消息。
    """
    if not push_config.get("SMTP_SERVER") or not push_config.get("SMTP_SSL") or not push_config.get(
            "SMTP_EMAIL") or not push_config.get("SMTP_PASSWORD") or not push_config.get("SMTP_NAME"):
        log.info(
    ...
    message['From'] = formataddr(
        (Header(push_config.get("SMTP_NAME"), 'utf-8').encode(), push_config.get("SMTP_EMAIL")))
...
def pushplus_bot(title: str, content: str) -> None:
    ...
    try:
        ...
        if response["code"] != 200:
            ...
            response = requests.post(url=url_old, json=payload).json()
        if response["code"] == 200:
            log.info("PUSHPLUS 推送成功！")
        else:
            log.error("PUSHPLUS 推送失败！")
    except Exception as ex:
        log.error(f"PUSHPLUS 推送出错！ ：{repr(ex)}")
...
if push_config.get("SMTP_SERVER") and push_config.get("SMTP_SSL") and push_config.get("SMTP_EMAIL") and push_config.get(
        "SMTP_PASSWORD") and push_config.get("SMTP_NAME"):
