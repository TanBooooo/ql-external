...
class StarNetworkPracticeGame(QLTask):
    ...
    def task(self, index, text, api_url):
        ...
        log.info(f"【{index}】{email}----正在参与锦标赛")
        ...
        for game in self.games:
            ...
            for i in range(3):
                try:
                    ...
                    payload = {"game": game, "mode": "tournament", "score": score, "extra": False,
                               "timestamp": timestamp, "key": encrypt_key(game, "tournament", score, timestamp)}
                    ...
                    if resp.text.count('id') > 0 and resp.text.count('SAVED') > 0:
                        log.info(f'【{index}】{email}----【{game}】锦标赛参与成功')
                    ...
                    if i != 2:
                        ...
                        log.info(f'【{index}】{email}----重试完毕----【{game}】锦标赛参与出错：{repr(ex)}')
                        self.fail_email.append(f'【{index}】{email}----【{game}】锦标赛参与出错：{repr(ex)}')
